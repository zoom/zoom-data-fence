...
public class TestSnowflakeConnectionSystemTest extends SnowflakeSysTestBase {
  ...
  public void beforeGroupKeyTest(ITestContext ctx)
    ...
    String userName =
        String.format("TEST_USER_%s", UUID.randomUUID().toString().replace("-", "")).toUpperCase();
    ...
    if (!Path.of(systemTmpDirLocation).toFile().isDirectory()) {
      throw new FileNotFoundException(
          String.format("System tmp dir %s does not exist", systemTmpDirLocation));
    }
    ...
    UserAttributes userAttributes =
        new UserAttributes(userName, networkPolicy, password, privateKeyPath, keyPair);
  ...
  public String getCurrentUserNetworkPolicy() {
    ...
    try (Connection connection = this.sysadminSnowflakeConnectionProvider.getConnection()) {
      ...
      PreparedStatement getNetworkPolicyStatement =
  ...
  public Path writePrivateKeyToPemFile(KeyPair keyPair, Path tmpDir) {
    ...
    try {
      ...
      throw new RuntimeException(
          String.format("Unable to write key to pem file %s.", outputFile), e);
    }
    return outputPath;
  }
  ...
  public void testPasswordAuth(ITestContext ctx) throws SQLException {
    UserAttributes userAttributes =
        (UserAttributes) Objects.requireNonNull(ctx.getAttribute("user-attributes"));
    ...
    snowflakeProviderConfigModel.setConnectionString(
    ...
    SnowflakeConnectionService snowflakeConnectionService =
        new SnowflakeConnectionService(snowflakeProviderConfigModel);
  ...
  public void testPatAuth(ITestContext ctx) throws SQLException, InterruptedException {
    ...
    try (Connection connection = this.securityadminSnowflakeConnectionProvider.getConnection()) {
      ...
      grantRoleStatement.executeUpdate(
          String.format("GRANT ROLE %s to USER %s", this.snowflakeSecurityAdminRole, userName));
      Thread.sleep(1000);
      Statement statement = connection.createStatement();
      String sql =
          String.format(
    ...
    if (token == null) {
      throw new RuntimeException(
          String.format("Unable to get programmatic access token for user %s.", userName));
    }
    SnowflakeConnectionProperties snowflakeConnectionProperties =
        new SnowflakeConnectionProperties();
    snowflakeConnectionProperties.setUser(userName);
    ...
    try (Connection connection =
        snowflakeConnectionService.connection(SnowflakeRoleType.SECURITYADMIN)) {
      connection.createStatement().execute("SELECT 1");
    }
  }
  ...
  public void testPrivateKeyFileAuth(ITestContext ctx) throws SQLException, IOException {
    ...
    snowflakeConnectionProperties.setSecurityAdminRole("PUBLIC");
    snowflakeConnectionProperties.setSysAdminRole("PUBLIC");
    snowflakeConnectionProperties.setPrivateKeyFile(
        userAttributes.privateKeyPath.toAbsolutePath().toString());
    SnowflakeProviderConfigModel snowflakeProviderConfigModel = new SnowflakeProviderConfigModel();
    snowflakeProviderConfigModel.setConnectionProperties(snowflakeConnectionProperties);
    snowflakeProviderConfigModel.setConnectionString(
        String.format("jdbc:snowflake://%s.snowflakecomputing.com/", snowflakeAccount));
