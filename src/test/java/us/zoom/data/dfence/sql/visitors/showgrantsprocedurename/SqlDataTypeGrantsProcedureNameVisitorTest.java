...
import us.zoom.data.dfence.sql.parser.ShowGrantsProcedureNameLexer;
...
class SqlDataTypeGrantsProcedureNameVisitorTest {
  ...
  public static Stream<DataTypeTestParams> visitDataTypeTestParams() {
    return Stream.of(
        ...
        new DataTypeTestParams(
            "TABLE(FLOAT)", new SqlDataType("TABLE", List.of(new SqlDataType("FLOAT")), List.of())),
        new DataTypeTestParams(
            "TABLE(FLOAT, NUMBER)",
            new SqlDataType(
                "TABLE", List.of(new SqlDataType("FLOAT"), new SqlDataType("NUMBER")), List.of())),
        new DataTypeTestParams(
            "TABLE(ARG_C FLOAT)",
  ...
  void visitDataType(DataTypeTestParams params) {
    ShowGrantsProcedureNameLexer lexer =
        new ShowGrantsProcedureNameLexer(CharStreams.fromString(params.input));
