...
class ObjectNameTest {
  ...
  static Stream<SplitPartsTestArg> splitObjectNameFixture() {
    return Stream.of(
        ...
        new SplitPartsTestArg(
            "FOO_ZOO.BAR_ZAR.GET_USER_NETPOLICY()",
            List.of("FOO_ZOO", "BAR_ZAR", "GET_USER_NETPOLICY()")),
        new SplitPartsTestArg(
            "FOO_ZOO.BAR_ZAR.GET_USER_NETPOLICY(VARCHAR, TABLE(VARCHAR, NUMBER))",
            List.of("FOO_ZOO", "BAR_ZAR", "GET_USER_NETPOLICY(VARCHAR, TABLE(VARCHAR, NUMBER))")));
  }
  ...
  void normalizeObjectName(String input, String expected) {
    // I am not a mean person. However, if I wake up evil some day I will make people write code to
    // pass this
