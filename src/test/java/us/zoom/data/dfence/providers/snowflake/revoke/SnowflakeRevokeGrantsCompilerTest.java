...
class SnowflakeRevokeGrantsCompilerTest {
  ...
  @Test
  void compileRevokeGrants_whenDatabaseAllGrantMatchesPlaybook_shouldRevoke() {
    // Given
    PlaybookPrivilegeGrant playbookGrant =
        createPlaybookGrant(
            "SCHEMA", "TEST_DB", "TEST_SCHEMA", null, List.of("USAGE"), true, true, true);
    SnowflakeGrantModel allGrant =
        createAllGrant("USAGE", "SCHEMA", "TEST_DB.TEST_SCHEMA", "ROLE", "TEST_ROLE");
    Map<String, SnowflakeGrantBuilder> currentGrants = createCurrentGrants(allGrant);
    ...
    // (TEST_DB, TEST_SCHEMA) is a valid standard target for SCHEMA (qualLevel 2 requires both
    // database and schema), so the grant type is STANDARD (not FUTURE_AND_ALL).
    // STANDARD grants only match STANDARD grants, so the ALL grant should be revoked
    assertEquals(1, actualRevokes.size());
    SnowflakeGrantModel revokedGrant = actualRevokes.get(0).getGrant();
    assertEquals("USAGE", revokedGrant.privilege());
    assertEquals("SCHEMA", revokedGrant.grantedOn());
  ...
  void compileRevokeGrants_whenDatabaseAllGrantDoesNotMatchPlaybook_shouldRevoke() {
    ...
    PlaybookPrivilegeGrant playbookGrant =
        createPlaybookGrant(
            "SCHEMA", "TEST_DB", "TEST_SCHEMA", null, List.of("USAGE"), true, true, true);
    SnowflakeGrantModel allGrant =
        createAllGrant("USAGE", "SCHEMA", "OTHER_DB.TEST_SCHEMA", "ROLE", "TEST_ROLE");
    Map<String, SnowflakeGrantBuilder> currentGrants = createCurrentGrants(allGrant);
