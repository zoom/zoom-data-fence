...
class SnowflakeProviderTest {
  ...
  @Mock SnowflakeGrantsService snowflakeGrantsService;

  @Mock SnowflakeStatementsService snowflakeStatementsService;

  @Mock SnowflakeObjectsService snowflakeObjectsService;
  ...
  public static Stream<CompileRoleChangesTestParams> compileRoleChangesTestParamsStream() {
    List<PlaybookPrivilegeGrant> playbookPrivilegeGrants =
        List.of(
            ...
            new PlaybookPrivilegeGrant(
                ...
                true),
            new PlaybookPrivilegeGrant(
                "database", null, null, "mock_db_name", List.of("usage"), true, true),
            new PlaybookPrivilegeGrant(
                "view", "*", "*", "mock_db_name", List.of("select"), true, true),
            new PlaybookPrivilegeGrant(
                "account", null, null, null, List.of("monitor"), false, false));
    String roleName = "mock_role_name";
    String roleId = "mock-role-id";
    List<List<String>> expectedRoleGrantStatements =
        List.of(
            List.of("GRANT MONITOR ON ACCOUNT TO ROLE MOCK_ROLE_NAME;"),
            List.of(
                "GRANT SELECT ON TABLE \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\".\"mock_table_name_2\" TO ROLE MOCK_ROLE_NAME;"),
            List.of(
                "GRANT SELECT ON TABLE \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\".\"MOCK_TABLE_NAME\" TO ROLE MOCK_ROLE_NAME;"),
            List.of(
                "GRANT SELECT ON FUTURE VIEWS IN DATABASE \"MOCK_DB_NAME\" TO ROLE MOCK_ROLE_NAME;"),
            List.of(
                "GRANT SELECT ON FUTURE VIEWS IN SCHEMA \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\" TO ROLE MOCK_ROLE_NAME;"),
            List.of(
                "GRANT UPDATE ON TABLE \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\".\"MOCK_TABLE_NAME\" TO ROLE MOCK_ROLE_NAME;"),
            List.of("GRANT USAGE ON DATABASE \"MOCK_DB_NAME\" TO ROLE MOCK_ROLE_NAME;"));
    List<String> expectedRoleCreationStatements =
        List.of("CREATE ROLE IF NOT EXISTS MOCK_ROLE_NAME;");
    SnowflakeGrantBuilderOptions extraGrantOptions = new SnowflakeGrantBuilderOptions();
    extraGrantOptions.setSuppressErrors(false);
    SnowflakeGrantBuilder extraGrant =
    ...
    return Stream.of(
        new CompileRoleChangesTestParams(
            ...
            new HashMap<>(),
            new CompiledChanges(
                roleId, roleName, expectedRoleCreationStatements, expectedRoleGrantStatements)),
        new CompileRoleChangesTestParams(
            new PlaybookRoleModel(roleName, playbookPrivilegeGrants),
            roleId,
            true,
  ...
  public static Stream<CompilePlaybookPrivilegeGrantsTestParams>
      compilePlaybookPrivilegeGrantsTestParamsStream() {
    return compileRoleChangesTestParamsStream()
        .filter(p -> p.playbookRoleModel().create() || p.roleExists)
        .map(
            p ->
                new CompilePlaybookPrivilegeGrantsTestParams(
  ...
  void setUp() {
    ...
    when(snowflakeObjectsService.objectExists(anyString(), eq(SnowflakeObjectType.DATABASE)))
        .thenAnswer(invocation -> databasesInAccount.contains(invocation.getArgument(0)));
    when(snowflakeObjectsService.objectExists(anyString(), eq(SnowflakeObjectType.SCHEMA)))
        .thenAnswer(invocation -> schemasInDatabase.contains(invocation.getArgument(0)));
    when(snowflakeObjectsService.objectExists(anyString(), eq(SnowflakeObjectType.TABLE)))
        .thenAnswer(invocation -> tablesInSchema.contains(invocation.getArgument(0)));
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.ACCOUNT, SnowflakeObjectType.DATABASE, ""))
        .thenReturn(databasesInAccount);
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.DATABASE, SnowflakeObjectType.SCHEMA, databaseName))
        .thenReturn(schemasInDatabase);
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.SCHEMA, SnowflakeObjectType.TABLE, databaseName + "." + schemaName))
        .thenReturn(tablesInSchema);
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.DATABASE, SnowflakeObjectType.TABLE, databaseName))
        .thenReturn(tablesInSchema);
  }
  ...
  void compileChanges(CompileRoleChangesTestParams params) {
    when(snowflakeGrantsService.getGrants(params.playbookRoleModel.name(), false))
        .thenReturn(params.mockGetGrantsResult);
    when(snowflakeObjectsService.getContainerObjectQualNames(
            eq(SnowflakeObjectType.ACCOUNT), eq(SnowflakeObjectType.ROLE), anyString()))
        .thenReturn(params.existingRoles);
    when(snowflakeObjectsService.objectExists(anyString(), eq(SnowflakeObjectType.ACCOUNT)))
        .thenReturn(true);
    when(snowflakeObjectsService.objectExists("OTHER_DB.OTHER_SCHEMA", SnowflakeObjectType.SCHEMA))
        .thenReturn(true);
    PlaybookModel playbookModel =
        new PlaybookModel(Map.of(params.roleId, params.playbookRoleModel));
    List<CompiledChanges> compiledChangesExpected =
        params.expected.containsChanges() ? List.of(params.expected) : List.of();
    List<CompiledChanges> compiledChangesActual =
        snowflakeProvider.compileChanges(playbookModel, false);
  ...
  void compileChangesOwnershipFilter() {
    when(snowflakeObjectsService.objectExists(
            "MOCK_DB_NAME.MOCK_SCHEMA_NAME.OTHER_TABLE_NAME", SnowflakeObjectType.TABLE))
        .thenReturn(true);
    PlaybookModel playbookModel =
        new PlaybookModel(
            Map.of(
                "role-a",
                new PlaybookRoleModel("role_a", List.of()),
                "role-b",
                new PlaybookRoleModel(
                    "role_b",
                    List.of(
                        new PlaybookPrivilegeGrant(
                            ...
                            true,
                            true)))));
    SnowflakeGrantBuilder extraOwnerGrantSilence =
        SnowflakeGrantBuilder.fromGrant(
            new SnowflakeGrantModel(
                "OWNERSHIP",
                "TABLE",
                "MOCK_DB_NAME.MOCK_SCHEMA_NAME.MOCK_TABLE_NAME",
                ...
                false,
                false));
    SnowflakeGrantBuilder extraOwnerGrantRevoke =
        SnowflakeGrantBuilder.fromGrant(
            new SnowflakeGrantModel(
                "OWNERSHIP",
                "TABLE",
                ...
                false,
                false));
    when(snowflakeGrantsService.getGrants("role_a", false))
        .thenReturn(
            Map.of(
    ...
    when(snowflakeObjectsService.getContainerObjectQualNames(
            eq(SnowflakeObjectType.ACCOUNT), eq(SnowflakeObjectType.ROLE), anyString()))
        .thenReturn(List.of("ROLE_A", "ROLE_B"));
    List<CompiledChanges> compiledChangesExpected =
        List.of(
            new CompiledChanges(
                "role-a",
  ...
  void compileChangesOwnershipFilterContainer() {
    when(snowflakeObjectsService.objectExists(
            "OTHER_DB_NAME.MOCK_SCHEMA_NAME.MOCK_TABLE_NAME", SnowflakeObjectType.TABLE))
        .thenReturn(true);
    PlaybookModel playbookModel =
        new PlaybookModel(
            Map.of(
                ...
                new PlaybookRoleModel(
                    ...
                    List.of(
                        new PlaybookPrivilegeGrant(
                            "table", "*", "*", databaseName, List.of("OWNERSHIP"), false, true)))));
    SnowflakeGrantBuilder extraOwnerGrantSilence =
        SnowflakeGrantBuilder.fromGrant(
            new SnowflakeGrantModel(
                "OWNERSHIP",
    ...
    List<CompiledChanges> compiledChangesExpected =
        List.of(
            ...
            new CompiledChanges(
                ...
                List.of(
                    List.of(
                        "GRANT OWNERSHIP ON TABLE \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\".\"mock_table_name_2\" TO ROLE ROLE_B COPY CURRENT GRANTS;"),
                    List.of(
                        "GRANT OWNERSHIP ON TABLE \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\".\"MOCK_TABLE_NAME\" TO ROLE ROLE_B COPY CURRENT GRANTS;"))));
    List<CompiledChanges> compiledChangesActual =
        snowflakeProvider.compileChanges(playbookModel, false);
        ...
        List.of(
            new CompiledChanges(
                ...
                List.of(
                    ...
                    "GRANT OWNERSHIP ON ROLE ROLE_A TO ROLE SECURITYADMIN COPY CURRENT GRANTS;"),
                List.of()));
    List<CompiledChanges> compiledChangesActual =
        snowflakeProvider.compileChanges(playbookModel, false);
    assertEquals(compiledChangesExpected, compiledChangesActual);
  ...
  void applyPrivilegeChanges() {
    List<CompiledChanges> compiledChanges =
        List.of(compileRoleChangesTestParamsStream().findFirst().get().expected);
    snowflakeProvider.applyPrivilegeChanges(compiledChanges);
    compiledChanges.forEach(
        x ->
  ...
  void applyPrivilegeChangesToRole() {
    CompiledChanges compiledChanges =
        compileRoleChangesTestParamsStream().findFirst().get().expected;
  ...
  void applyRolesChanges() {
    ...
    compiledChanges.forEach(
        x ->
            verify(snowflakeStatementsService, times(1))
                .applyStatements(x.roleCreationStatements()));
  }
  ...
  void applyRoleChanges() {
    ...
    verify(snowflakeStatementsService, times(1))
        .applyStatements(compiledChanges.roleCreationStatements());
  }
  ...
  void compileRoleChanges(CompileRoleChangesTestParams params) {
    ...
    when(snowflakeObjectsService.objectExists("OTHER_DB.OTHER_SCHEMA", SnowflakeObjectType.SCHEMA))
        .thenReturn(true);
    CompiledChanges compiledChangesActual =
        snowflakeProvider.compileRoleChanges(
            params.roleId,
  ...
  void compilePlaybookPrivilegeGrants(CompilePlaybookPrivilegeGrantsTestParams params) {
    ...
    when(snowflakeObjectsService.objectExists("OTHER_DB.OTHER_SCHEMA", SnowflakeObjectType.SCHEMA))
        .thenReturn(true);
    when(snowflakeGrantsService.getGrants(params.roleName(), false))
        .thenReturn(params.mockGetGrantsResult);
    List<List<String>> actualStatements =
        this.snowflakeProvider.compilePlaybookPrivilegeGrants(
    ...
    List<PlaybookPrivilegeGrant> playbookPrivilegeGrants =
        List.of(
            new PlaybookPrivilegeGrant(
                ...
                true,
                true));
    List<List<String>> expectedStatements =
        List.of(
            List.of("GRANT SELECT ON FUTURE VIEWS IN DATABASE \"MOCK_DB_NAME\" TO ROLE FOO_BAR;"),
            List.of(
                "GRANT SELECT ON FUTURE VIEWS IN SCHEMA \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\" TO ROLE FOO_BAR;"),
            List.of("GRANT UPDATE ON FUTURE VIEWS IN DATABASE \"MOCK_DB_NAME\" TO ROLE FOO_BAR;"),
            List.of(
                "GRANT UPDATE ON FUTURE VIEWS IN SCHEMA \"MOCK_DB_NAME\".\"MOCK_SCHEMA_NAME\" TO ROLE FOO_BAR;"));
    List<List<String>> actualStatements =
        this.snowflakeProvider.compilePlaybookPrivilegeGrants(
            playbookPrivilegeGrants,
            roleName,
  ...
  void playbookGrantToSnowflakeGrants() {
    ...
    List<SnowflakeGrantBuilder> expectedBuilders =
        List.of(
            new SnowflakePermissionGrantBuilder(
                new SnowflakeGrantModel(
                    "SELECT",
    ...
    List<SnowflakeGrantBuilder> actualBuilders =
        this.snowflakeProvider.playbookGrantToSnowflakeGrants(
            playbookPrivilegeGrant, roleName, options);
  ...
  void playbookGrantToSnowflakeGrantsApplicationRole() {
    ...
    List<SnowflakeGrantBuilder> expectedBuilders =
        List.of(
            new SnowflakeApplicationRoleGrantBuilder(
                new SnowflakeGrantModel(
                    "USAGE",
                    "APPLICATION_ROLE",
                    "SNOWFLAKE.TRUST_CENTER_VIEWER",
  ...
  void standardGrants() {
    PlaybookPrivilegeGrant playbookPrivilegeGrant =
        new PlaybookPrivilegeGrant(
            ...
            true);
    String roleName = "mock_role_name";
    List<SnowflakeGrantModel> expectedGrants =
        List.of(
            new SnowflakeGrantModel(
                "SELECT",
                "TABLE",
            ...
            new SnowflakeGrantModel(
                ...
                false,
                false));
    List<SnowflakeGrantModel> actualGrants =
        this.snowflakeProvider.standardGrants(playbookPrivilegeGrant, roleName);
  ...
  void containerGrants() {
    ...
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.DATABASE, SnowflakeObjectType.SCHEMA, "MOCK_DB_NAME"))
        .thenReturn(List.of("MOCK_DB_NAME.MOCK_SCHEMA_NAME"));
    when(snowflakeObjectsService.getContainerObjectQualNames(
            SnowflakeObjectType.SCHEMA, SnowflakeObjectType.TABLE, "MOCK_DB_NAME.MOCK_SCHEMA_NAME"))
        .thenReturn(tablesInDatabase);
    String roleName = "mock_role_name";
    ...
    List<SnowflakeGrantModel> actualGrants =
        this.snowflakeProvider.containerGrants(playbookPrivilegeGrant, roleName);
    assertEquals(expectedGrants, actualGrants);
  }
  ...
  void createFutureGrants() {
    List<SnowflakeGrantModel> expected =
        List.of(
            new SnowflakeGrantModel(
                "SELECT",
                "EXTERNAL_TABLE",
                ...
                true,
                false));
    List<SnowflakeGrantModel> actual =
        snowflakeProvider.createFutureGrants(
            SnowflakeObjectType.EXTERNAL_TABLE, "FOO_DB", List.of("SELECT"), "BAR_ROLE", false);
