...
class SnowflakeGrantsServiceTest {
  ...
  @Mock Connection snowflakeConnection;

  @InjectMocks SnowflakeGrantsService snowflakeGrantsService;
  ...
  void getGrants() throws SQLException {
    ...
    try (Statement currentGrantsStatement = Mockito.mock(Statement.class)) {
      try (Statement futureGrantsStatement = Mockito.mock(Statement.class)) {
        when(snowflakeConnection.createStatement())
            .thenReturn(currentGrantsStatement, futureGrantsStatement);
        List columnNames =
            List.of(
                "privilege", "granted_on", "name", "granted_to", "grantee_name", "grant_option");
        when(currentGrantsStatement.getResultSet())
            .thenReturn(
                new MockResultSet(
                    List.of(
                        List.of(
                        ...
                        List.of(
                            ...
                            new I(false))),
                    columnNames));
        when(futureGrantsStatement.getResultSet())
            .thenReturn(new MockResultSet(List.of(), columnNames));
        ...
        Map<String, SnowflakeGrantBuilder> actual =
            snowflakeGrantsService.getGrants(roleName, skipUnkownTypes);
                ...
                new MockResultSet(
                    List.of(
                        ...
                        List.of(
                            ...
                            new I(
                                "DEV_ZOOM_DATA.STREAMLIT_APPS.\"DIRECTORY_TB_\"\"FOV76IX1A0QDJ719 (Stage)\"\"_34537889316558814\""),
                            new I("ROLE"),
                            new I("MOCK_ROLE"),
                            new I(false))),
                    columnNames));
