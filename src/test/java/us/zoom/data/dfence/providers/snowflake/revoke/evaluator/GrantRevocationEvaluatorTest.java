...
class GrantRevocationEvaluatorTest {
  ...
  @Test
  void needsRevoke_shouldReturnFalse_whenGrantModelHasInvalidObjectType() {
    // Critical: Invalid object types should be handled gracefully
    PlaybookGrantHashIndex index = createEmptyIndex();
    GrantRevocationEvaluator evaluator = new GrantRevocationEvaluator(index);
    SnowflakeGrantModel grantToCheck =
    ...
    assertFalse(
        actualNeedsRevoke,
        "Should return false (don't revoke) when grant model has invalid object type");
  }
  ...
  @Test
  void needsRevoke_shouldReturnFalse_whenExceptionOccursDuringEvaluation() {
    // Critical: Exception handling branch - when conversion fails, should return false (don't
    // revoke)
    ...
    assertFalse(
        actualNeedsRevoke,
        "Should return false (don't revoke) when exception occurs during evaluation");
  }
  ...
  private PlaybookGrant createPlaybookGrant(
    ...
    return new PlaybookGrant(
        ...
        List.of(new GrantPrivilege(priv)),
