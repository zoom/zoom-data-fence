...
class PlaybookModelTest {
    ...
    void testPlaybookModelWithUnsupportedRevokeBehavior() {
        PlaybookModel playbook = new PlaybookModel(
    ...
    void testPlaybookModelMergeWithSameUnsupportedRevokeBehavior() {
        PlaybookModel playbook1 = new PlaybookModel(
                Map.of("role-1", new PlaybookRoleModel("role_1", List.of())),
                "SECURITYADMIN",
                UnsupportedRevokeBehavior.IGNORE
        );
        
        PlaybookModel playbook2 = new PlaybookModel(
    ...
    void testPlaybookModelMergeWithDifferentUnsupportedRevokeBehavior() {
        ...
        RbacDataError error = assertThrows(RbacDataError.class, () -> {
            PlaybookModel.merge(List.of(playbook1, playbook2));
        });
        
        assertTrue(error.getMessage().contains("Unsupported revoke behavior is declared more than once with values"));
    }
    ...
    void testPlaybookModelMergeWithDuplicateRoleKeys() {
        ...
        assertTrue(error.getMessage().contains("Cannot merge playbook models. The following role keys are duplicates"));
    }
    ...
    void testPlaybookModelImmutability() {
        Map<String, PlaybookRoleModel> originalRoles = Map.of("test-role", new PlaybookRoleModel("test_role", List.of()));
        PlaybookModel playbook = new PlaybookModel(originalRoles, "SECURITYADMIN", UnsupportedRevokeBehavior.DROP);
