...
class PlaybookEnhancedTest {
    ...
    PlaybookModel playbookModel =
        new PlaybookModel(
            Map.of(
                ...
                new PlaybookRoleModel(
                    "role_without_behavior", List.of(), true, true, true, null, null)),
            "SECURITYADMIN",
  ...
  void testPropagateDefaultsWithBothRoleOwnerAndUnsupportedRevokeBehavior() {
    PlaybookModel playbookModel =
        new PlaybookModel(
            Map.of(
                ...
                "role-without-both",
                new PlaybookRoleModel(
                    "role_without_both", List.of(), true, true, true, null, null)),
            "SECURITYADMIN",
            UnsupportedRevokeBehavior.IGNORE);
    ...
    assertEquals(
        UnsupportedRevokeBehavior.DROP,
        result.roles().get("role-with-both").unsupportedRevokeBehavior());

    // Role without both should inherit from playbook
    assertEquals("SECURITYADMIN", result.roles().get("role-without-both").roleOwner());
    assertEquals(
        UnsupportedRevokeBehavior.IGNORE,
        result.roles().get("role-without-both").unsupportedRevokeBehavior());
  }
