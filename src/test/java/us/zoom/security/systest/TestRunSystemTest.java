...
import us.zoom.security.dfence.test.fixtures.LifecycleWrapper;
import us.zoom.security.dfence.test.fixtures.SnowflakeConnectionProvider;
...
public class TestRunSystemTest extends SnowflakeSysTestBase {
    ...
    public void beforeGroupsA(ITestContext ctx) {
        String databaseName = String.format("TEST_DATABASE_%s", UUID.randomUUID().toString().replace("-", ""))
                .toUpperCase();
        String roleName = String.format("TEST_ROLE_%s", UUID.randomUUID().toString().replace("-", "")).toUpperCase();
        String roleName2 = String.format("TEST_ROLE2_%s", UUID.randomUUID().toString().replace("-", "")).toUpperCase();
        ...
        String procedureNameNoArgsShowGrantsQual = String.format("%s.%s.\"%s():VARCHAR\"", databaseName, schemaName, procedureNamePartial);
        ...
        lifecycleManager.getLifecycleObjects()
                .add(SnowflakeLifecycleObject.schema(sysadminSnowflakeConnectionProvider, databaseName, schemaName));
        lifecycleManager.getLifecycleObjects().add(SnowflakeLifecycleObject.table(
                sysadminSnowflakeConnectionProvider,
                databaseName,
                schemaName,
                tableName));
        lifecycleManager.getLifecycleObjects().add(LifecycleWrapper.builder().performSetup(false).exists(true)
                .wraps(SnowflakeLifecycleObject.role(securityadminSnowflakeConnectionProvider, roleName)).build());
        lifecycleManager.getLifecycleObjects().add(LifecycleWrapper.builder().performSetup(false).exists(true)
                .wraps(SnowflakeLifecycleObject.role(securityadminSnowflakeConnectionProvider, roleName2)).build());
        lifecycleManager.getLifecycleObjects().add(new SnowflakeLifecycleObject(
                sysadminSnowflakeConnectionProvider, String.format(
                """
                        ...
                        def run(session, from_table, to_table, count):
                          ...
                          return "SUCCESS"
                        $$;
                        """, databaseName, schemaName, procedureNamePartial), String.format(
    ...
    public void afterGroupsA(ITestContext ctx) throws SQLException {
        try (Connection connection = securityadminSnowflakeConnectionProvider.getConnection()) {
            connection.createStatement().execute(
                    String.format("GRANT OWNERSHIP ON ALL TABLES IN DATABASE %s TO ROLE %S REVOKE CURRENT GRANTS", ctx.getAttribute("database-name"), snowflakeSysadminRole)
            );
            connection.createStatement().execute(
                    String.format("GRANT OWNERSHIP ON ALL PROCEDURES IN DATABASE %s TO ROLE %S REVOKE CURRENT GRANTS", ctx.getAttribute("database-name"), snowflakeSysadminRole)
            );
        }
    ...
    public void testCompile(ITestContext ctx) throws URISyntaxException {
        ...
        try (Connection connection = securityadminSnowflakeConnectionProvider.getConnection()) {
            connection.createStatement().execute(
                    String.format("GRANT USAGE, MONITOR ON DATABASE %s TO ROLE %s", databaseName, snowflakeSysadminRole)
            );
            connection.createStatement().execute(
                    String.format("GRANT USAGE, MONITOR ON ALL SCHEMAS IN DATABASE %s TO ROLE %s", databaseName, snowflakeSysadminRole)
            );
        ...
        PlaybookService playbookService = new PlaybookServiceBuilder().setPlaybookYamlStrings(rolesFile)
                .putAllVariables(variables).setProfilesYamlString(profilesFile).build();
    ...
    public void testApply(ITestContext ctx) throws SQLException {
        ...
        String procedureNameShowGrantsQual = (String) ctx.getAttribute("procedure-name-show-grants-qual");
        String procedureNameNoArgsShowGrantsQual = (String) ctx.getAttribute("procedure-name-no-args-show-grants-qual");
        ...
        List<Grant> grantsExpected = new ArrayList<>(List.of(
                ...
                new Grant("USAGE", "SCHEMA", String.join(".", databaseName, schemaName), "ROLE", roleName),
                new Grant("MONITOR", "SCHEMA", String.join(".", databaseName, schemaName), "ROLE", roleName),
                new Grant("CREATE SEMANTIC VIEW", "SCHEMA", String.join(".", databaseName, schemaName), "ROLE", roleName),
                new Grant("SELECT", "TABLE", String.join(".", databaseName, schemaName, tableName), "ROLE", roleName),
                new Grant("OWNERSHIP", "TABLE", String.join(".", databaseName, schemaName, tableName), "ROLE", roleName),
                new Grant("USAGE", "APPLICATION_ROLE", "SNOWFLAKE.TRUST_CENTER_VIEWER", "ROLE", roleName),
                new Grant("USAGE", "PROCEDURE", procedureNameShowGrantsQual, "ROLE", roleName),
                new Grant("USAGE", "PROCEDURE", procedureNameNoArgsShowGrantsQual, "ROLE", roleName),
                new Grant("OWNERSHIP", "PROCEDURE", procedureNameShowGrantsQual, "ROLE", roleName),
                new Grant("OWNERSHIP", "PROCEDURE", procedureNameNoArgsShowGrantsQual, "ROLE", roleName)
        ));
    ...
    public void testCompileRevoke(ITestContext ctx) throws SQLException, URISyntaxException {
        String roleName = (String) Objects.requireNonNull(ctx.getAttribute("role-name"));
        Map<String, String> variables = (Map<String, String>) Objects.requireNonNull(ctx.getAttribute("variables"));
        try (Connection connection = securityadminSnowflakeConnectionProvider.getConnection()) {
            connection.createStatement().execute(
                    String.format("GRANT ROLE %s TO ROLE %s", roleName, snowflakeSysadminRole)
            );
        }
        File rolesFile = new File(Objects.requireNonNull(this.getClass().getClassLoader()
    ...
    public void testApplyRevoke(ITestContext ctx) throws SQLException, InterruptedException, JsonProcessingException {
        ...
        // Snowflake appears to have eventual consistency on this metadata after the ownership change. We need to wait
        ...
        while (LocalDateTime.now().isBefore(maxTime.plusSeconds(10)) && !success) {
            try {
                ...
                List<Grant> grantsExpected2 = new ArrayList<>(List.of(
                        new Grant("OWNERSHIP", "TABLE", String.join(".", databaseName, schemaName, tableName), "ROLE", roleName2),
                        new Grant("OWNERSHIP", "PROCEDURE", procedureNameShowGrantsQual, "ROLE", roleName2),
                        new Grant("OWNERSHIP", "PROCEDURE", procedureNameNoArgsShowGrantsQual, "ROLE", roleName2)
                ));
