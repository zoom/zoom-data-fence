...
import us.zoom.data.dfence.providers.snowflake.grant.current.CurrentGrantsProvider;
import us.zoom.data.dfence.providers.snowflake.grant.desired.create.DesiredGrantsProvider;
import us.zoom.data.dfence.providers.snowflake.grant.diff.GrantsDiffProvider;
...
public class SnowflakeProvider implements Provider {
    ...
    private final CurrentGrantsProvider currentGrantsProvider;
    private final GrantsDiffProvider grantsDiffProvider;
    ...
    public SnowflakeProvider(
        ...
        this.desiredGrantsProvider = new DesiredGrantsProvider(snowflakeObjectsService);
        this.grantsDiffProvider = new GrantsDiffProvider(desiredGrantsProvider, forkJoinPool);
        this.currentGrantsProvider = new CurrentGrantsProvider(snowflakeGrantsService);
    }
    ...
    public PartitionedGrantStatements compilePlaybookPrivilegeGrants(
        ...
        try {
            ...
            Map<String, SnowflakeGrantBuilder> customGrantBuilders = currentGrantsProvider.getAllCurrentGrantsForRole(
                    new LoadAllCurrentGrantsForRoleSettings(
                            roleExists,
                            roleName,
                            revokeOtherGrants,
                            ignoreUnknownGrants
            ...
            MapDifference<String, SnowflakeGrantBuilder> diff = grantsDiffProvider.diff(
                    new AllDesiredGrantsCreationDataForRole(
                            privilegeGrants,
                            roleName,
                            options
                    ),
            ...
            if (revokeOtherGrants) {
                ...
                List<SnowflakeGrantBuilder> grantsToRevoke = SnowflakeRevokeGrantsCompiler
                        .compileRevokeGrants(privilegeGrants, customGrantBuilders).stream().toList();
    ...
    private PartitionedGrantStatements partitionGrantsByOwnership(GrantBuilderDiff grantBuilderDiff) {
        ...
        List<SnowflakeGrantBuilder> ownershipGrantBuilders = partitionedGrantGrants.getOrDefault(true, new ArrayList<>());
        List<SnowflakeGrantBuilder> nonOwnershipGrantBuilders = partitionedGrantGrants.getOrDefault(false, new ArrayList<>());
        List<SnowflakeGrantBuilder> ownershipRevokeBuilders = partitionedRevokeGrants.getOrDefault(true, new ArrayList<>());
        List<SnowflakeGrantBuilder> nonOwnershipRevokeBuilders = partitionedRevokeGrants.getOrDefault(false, new ArrayList<>());
