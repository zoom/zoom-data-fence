...
public class SnowflakeOwnedObjectFilter {
  ...
  public SnowflakeOwnedObjectFilter(
      ...
      String roleName) {
    this.databaseName =
        databaseName == null ? null : ObjectName.normalizeObjectNamePart(databaseName);
    this.schemaName =
        schemaName == null || "*".equals(schemaName)
            ? schemaName
            : ObjectName.normalizeObjectNamePart(schemaName);
    this.objectName =
        objectName == null || "*".equals(objectName)
            ? objectName
  ...
  public static List<SnowflakeOwnedObjectFilter> filtersFromPlaybookModel(PlaybookModel playbook) {
    return playbook.roles().values().stream()
        .flatMap(
            role ->
                role.grants().stream()
                    .filter(
                        grant ->
                            grant.privileges().stream().anyMatch("OWNERSHIP"::equalsIgnoreCase))
                    .map(
                        grant ->
                            new SnowflakeOwnedObjectFilter(
                                grant.databaseName(),
                                grant.schemaName(),
                                grant.objectName(),
  ...
  public static GrantBuilderDiff filterDiff(
      GrantBuilderDiff grantBuilderDiff, PlaybookModel playbookModel) {
    List<SnowflakeOwnedObjectFilter> filters =
        SnowflakeOwnedObjectFilter.filtersFromPlaybookModel(playbookModel);
    List<SnowflakeGrantBuilder> revokeGrants =
  ...
  Boolean keep(SnowflakeGrantModel snowflakeGrantModel) {
    // We do this in separate steps rather than one big boolean in order to return early before
    // instantiating anything else.
    if (!"OWNERSHIP".equals(snowflakeGrantModel.privilege())
        || ObjectName.equalObjectName(snowflakeGrantModel.granteeName(), roleName)
        || snowflakeGrantModel.future()
        || snowflakeGrantModel.all()) {
      return true;
    }
    SnowflakeObjectType grantObjectType =
        SnowflakeObjectType.valueOf(snowflakeGrantModel.grantedOn());
    ...
    try {
      ...
      throw new ObjectNameException(
          String.format(
              ...
              snowflakeGrantModel, snowflakeGrantModel.name()),
          e);
    }
    switch (grantObjectType.getQualLevel()) {
        ...
        if (!(grantObjectNameParts.size() == 1)) {
          throw new RbacDataError(
              String.format(
                  ...
                  snowflakeGrantModel.granteeName(), grantObjectType));
        }
      ...
      case 2 -> {
        ...
        return !(ObjectName.equalObjectName(databaseName, grantObjectNameParts.get(0))
            && ("*".equals(schemaName)
                || ObjectName.equalObjectName(schemaName, grantObjectNameParts.get(1))));
      }
      case 3 -> {
        // We don't have to worry about future grants with a qual level of three and two grant parts
        // because we are already ignoring those.
        ...
        return !(ObjectName.equalObjectName(databaseName, grantObjectNameParts.get(0))
            && (("*".equals(objectName) && "*".equals(schemaName))
                || ("*".equals(objectName)
                    && ObjectName.equalObjectName(schemaName, grantObjectNameParts.get(1)))
                || (ObjectName.equalObjectName(schemaName, grantObjectNameParts.get(1))
                    && ObjectName.equalObjectName(objectName, grantObjectNameParts.get(2)))));
      }
      default -> {
        throw new RbacDataError(
            String.format(
