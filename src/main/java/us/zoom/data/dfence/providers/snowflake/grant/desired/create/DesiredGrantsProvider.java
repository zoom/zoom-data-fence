...
public class DesiredGrantsProvider {
  ...
  private List<SnowflakeGrantModel> getGrants(PlaybookGrant grant, String roleName) {
    ...
    if (data instanceof GrantsCreationData.Standard s) {
      ...
      return switch (c.options()) {
        case FUTURE_AND_ALL -> resolveFutureAndAllGrants(c);
        case FUTURE -> futureGrantsProvider.createGrants(c);
        case ALL -> allGrantsProvider.createGrants(c);
      };
    } else {
      log.error("Unknown grant creation plan type: {} for role {}", data.getClass(), roleName);
      throw new RbacDataError("Unknown grant creation plan type: " + data.getClass());
    }
  }

  private List<SnowflakeGrantModel> resolveFutureAndAllGrants(GrantsCreationData.Container plan) {
    List<SnowflakeGrantModel> grants = new ArrayList<>();
    grants.addAll(futureGrantsProvider.createGrants(plan));
    grants.addAll(allGrantsProvider.createGrants(plan));
    return grants;
  }
}
