...
public class SnowflakeObjectExistsFilter {
  public static GrantBuilderDiff objectExistsGrantBuilderDiffFilter(
    ...
    List<String> playbookRoleNames =
        playbookModel.roles().values().stream()
            .map(x -> ObjectName.normalizeObjectName(x.name()))
            .toList();
    return new GrantBuilderDiff(
        grantBuilderDiff.grant().stream()
            .filter(
                grantBuilder ->
                    objectExistsGrantFilter(
                        grantBuilder.getGrant(), snowflakeObjectsService, playbookRoleNames))
            .toList(),
  ...
  public static Boolean objectExistsGrantFilter(
      ...
      SnowflakeObjectsService snowflakeObjectsService,
      List<String> playbookRoleNames) {
    if (snowflakeGrantModel.grantedOn().equals("ROLE")
        && playbookRoleNames.contains(ObjectName.normalizeObjectName(snowflakeGrantModel.name()))) {
      return true;
    }
      ...
      switch (objectNameParts.size()) {
        ...
        default -> {
          throw new RbacDataError(
              String.format(
                  "Invalid object name %s for future or all grant.", snowflakeGrantModel.name()));
        }
      }
    } else {
    ...
    try {
      ...
      throw new ObjectNameException(
          String.format(
              "Unable to check if %s %s exists due to object name exception.",
              snowflakeObjectType, objectName),
          e);
    }
    if (!ret) {
      log.info(
          "{} {} not found. Skipping.", snowflakeObjectType.toString().toLowerCase(), objectName);
    }
    return ret;
  }
}
