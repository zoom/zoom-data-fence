...
public class SnowflakeObjectsService {
  ...
  private final Map<String, List<String>> getContainerObjectQualNamesCache =
  ...
  public SnowflakeObjectsService(SnowflakeConnectionService snowflakeConnectionService) {
    this.snowflakeDefaultObjectService =
        new SnowflakeDefaultObjectService(snowflakeConnectionService);
    this.snowflakeTableObjectService = new SnowflakeTableObjectService(snowflakeConnectionService);
    this.snowflakeStageObjectsService =
        new SnowflakeStageObjectsService(snowflakeConnectionService);
    this.snowflakeProcedureObjectService =
        new SnowflakeProcedureObjectService(snowflakeConnectionService);
    this.snowflakeFunctionObjectService =
        new SnowflakeFunctionObjectService(snowflakeConnectionService);
  }
  ...
  public List<String> getContainerObjectQualNames(
    ...
    String key =
        String.join(
            "::",
            List.of(
                containerObjectType.getObjectType(), objectType.getObjectType(), containerName));
    if (getContainerObjectQualNamesCache.containsKey(key)) {
      return getContainerObjectQualNamesCache.get(key);
    } else {
      List<String> result =
          getContainerObjectQualNamesRaw(containerObjectType, objectType, containerName);
  ...
  public Boolean objectExistsRaw(String objectName, SnowflakeObjectType objectType) {
    ...
    switch (objectType.getQualLevel()) {
      ...
      default ->
          throw new RuntimeException(
              String.format("Invalid qualification level of %s.", objectType.getQualLevel()));
    }
    ...
    List<String> containerObjectQualNames =
        getContainerObjectQualNames(containerObjectType, objectType, containerName);
  ...
  public List<String> getContainerObjectQualNamesRaw(
    ...
    switch (objectType) {
      case TABLE, VIEW, MATERIALIZED_VIEW, EXTERNAL_TABLE, EVENT_TABLE -> {
        return snowflakeTableObjectService.getContainerTables(
            containerName, containerObjectType, objectType);
      }
        // We will only consider external stages for now as a patch because internal and external
        // stages allow different types of grants.
      case STAGE -> {
        return snowflakeStageObjectsService.getContainerExternalStages(
            containerName, containerObjectType);
      }
      case PROCEDURE -> {
        return snowflakeProcedureObjectService.getContainerProcedures(
            containerName, containerObjectType);
      }
      case FUNCTION -> {
        return snowflakeFunctionObjectService.getContainerProcedures(
            containerName, containerObjectType);
      }
      default -> {
