...
public abstract class SnowflakeGrantBuilder {
  ...
  public static SnowflakeGrantBuilder fromGrant(
    ...
    SnowflakeGrantBuilder[] snowflakeGrantBuilders =
        new SnowflakeGrantBuilder[] {
  ...
  public abstract List<GrantValidationDefinition> getValidCombinations();

  public Boolean isValid() {
    return !this.getGrant().all()
        && !this.getGrant().future()
        && this.getValidCombinations().stream().anyMatch(x -> x.validate(getGrant()));
  }
  ...
  public PlaybookPrivilegeGrant playbookPrivilegeGrant() {
    ...
    SnowflakeObjectType snowflakeObjectType =
        SnowflakeObjectType.valueOf(this.getGrant().grantedOn());
    ...
    if (snowflakeObjectType.getQualLevel() >= 1) {
      if (snowflakeObjectType.getQualLevel() == 1
          && !snowflakeObjectType.equals(SnowflakeObjectType.DATABASE)) {
        objectName = parts.get(0);
      } else {
