...
public record SnowflakeGrantModel(
  ...
  public SnowflakeGrantModel {
    if (!name.isEmpty()) {
      // Patch for user name not quoted in Snowflake. This looks like a bug in Snowflake where they
      // handle the
      ...
      try {
        ...
        throw new ObjectNameException(
            String.format("Unable to normalize name %s for grant.", name), e);
      }
    }
    ...
    if (!granteeName.isEmpty()) {
      try {
        ...
        throw new ObjectNameException(
            String.format("Unable to normalize grantee name %s for grant.", granteeName), e);
      }
    }
  }
  ...
  public String getEscapedName() {
    try {
      ...
      throw new ObjectNameException(
          String.format("Unable to generate escaped name for grant %s", this, e));
    }
  }
}
