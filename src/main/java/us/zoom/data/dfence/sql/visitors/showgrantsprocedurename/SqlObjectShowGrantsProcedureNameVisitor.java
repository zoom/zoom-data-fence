...
public class SqlObjectShowGrantsProcedureNameVisitor
    extends ShowGrantsProcedureNameBaseVisitor<SqlObject> {
  ...
  @Override
  public SqlObject visitShowGrantsProcedureName(
      ShowGrantsProcedureNameParser.ShowGrantsProcedureNameContext ctx) {
    ...
    if (ctx.part() != null && !ctx.part().isEmpty()) {
      sqlObject
          .getNames()
          .addAll(
              ctx.part().stream().map(x -> x.accept(partShowGrantsProcedureNameVisitor)).toList());
    } else {
    ...
    if (ctx.callableName() != null && ctx.callableName().unquotedIdentifier() != null) {
      String procedureName =
          ctx.callableName().unquotedIdentifier().accept(partShowGrantsProcedureNameVisitor);
      sqlObject.getNames().add(procedureName);
    } else {
      throw new ParserException(
          String.format("Invalid callable name identifier '%s'.", ctx.callableName().getText()));
    }
    if (ctx.callableName() != null
        && ctx.callableName().expandedArgs() != null
        && !ctx.callableName().expandedArgs().isEmpty()) {
      sqlObject
          .getArguments()
          .addAll(
              ctx.callableName().expandedArgs().expandedArgument().stream()
                  .map(x -> x.accept(sqlDataTypeGrantsProcedureNameVisitor))
                  .toList());
    }
    return sqlObject;
  }
  ...
  public SqlObject visitCallableName(ShowGrantsProcedureNameParser.CallableNameContext ctx) {
    ...
    try {
      if (ctx.unquotedIdentifier() != null) {
        ...
        throw new ParserException(
            String.format("Invalid callable name identifier '%s'.", ctx.getText()));
      }
      if (ctx.expandedArgs() != null && !ctx.expandedArgs().isEmpty()) {
        sqlObject
            .getArguments()
            .addAll(
                ctx.expandedArgs().expandedArgument().stream()
                    .map(x -> x.accept(sqlDataTypeGrantsProcedureNameVisitor))
                    .toList());
      }
      return sqlObject;
      ...
      throw new ParserException(
          String.format("Failure to parse callable name '%s'.", ctx.getText()), e);
    }
  }
}
