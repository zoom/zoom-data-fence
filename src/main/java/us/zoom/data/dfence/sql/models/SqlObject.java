...
public class SqlObject {
  ...
  public String normalizedArguments() {
    ...
    if (hasArguments) {
      ...
      if (!this.arguments.isEmpty()) {
        List<String> argumentsNormalized =
            arguments.stream().map(SqlDataType::toNormalizedName).toList();
  ...
  public String quotedName() {
    return String.join(".", this.names.stream().map(this::_quotedStringPart).toList())
        + normalizedArguments();
  }
}
