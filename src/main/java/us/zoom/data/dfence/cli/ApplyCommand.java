...
@CommandLine.Command(name = "apply", mixinStandardHelpOptions = true, showDefaultValues = true)
@Slf4j
public class ApplyCommand extends PlaybookCommand {
  ...
  @CommandLine.Option(
      names = {"--auto-approve"},
      description = "Do not prompt for approval.",
      defaultValue = "false")
  private Boolean autoApprove;

  @CommandLine.Option(
      names = {"--changes-file", "-c"},
      description = "Existing compiled changes file to read output from.")
    ...
    if (changesFile != null) {
      ...
      try {
        ...
        throw new RbacDataError(
            String.format("Unable to read changes file %s. %s", changesFile.toString(), e), e);
      }
    } else {
