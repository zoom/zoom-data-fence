...
import us.zoom.data.dfence.ProviderBuilder;
...
public abstract class ProviderCommand extends ErrorHandledCallable {
  ...
  @CommandLine.Option(
      names = {"--profile-file"},
      description = "Connection profile file",
      defaultValue = "./profiles.yml")
  @Setter
  protected File profileFile;

  @CommandLine.Option(
      names = {"--profile", "-p"},
      description = "Profile name to use for the connection.")
  protected String profile;

  @CommandLine.Option(
      names = {"--threads"},
      description =
          "Number of threads to use for parallel operations. If not specified, the common ForkJoinPool will be used.")
  protected Integer threads;

  @Setter private ProviderBuilder providerBuilder = new ProviderBuilder();

  public Provider getProvider() {
    log.debug("Creating provider.");
    ProviderBuilder builder =
        providerBuilder
            .setProfilesYamlString(profileFile)
