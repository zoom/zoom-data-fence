...
public class Playbook {
  ...
  public static PlaybookModel parse(String value, Map<String, String> variables)
    ...
    try {
      ...
      throw new RbacDataError(
          String.format("Unable to parse playbook yaml string as a valid playbook. %s", e), e);
    }
  }

  public static PlaybookModel filterPlaybook(PlaybookModel playbookModel) {
    Map<String, PlaybookRoleModel> playbookRoleModelsFiltered =
        playbookModel.roles().keySet().stream()
    ...
    return new PlaybookModel(
        playbookRoleModelsFiltered,
        playbookModel.roleOwner(),
        playbookModel.unsupportedRevokeBehavior());
  }

  public static PlaybookRoleModel filterRole(PlaybookRoleModel playbookRoleModel) {
    List<PlaybookPrivilegeGrant> filteredGrants =
        playbookRoleModel.grants().stream().filter(PlaybookPrivilegeGrant::enable).toList();
  ...
  public static PlaybookModel propagateDefaults(PlaybookModel playbookModel) {
    Map<String, PlaybookRoleModel> roles =
        playbookModel.roles().keySet().stream()
            .collect(
                Collectors.toMap(
                    k -> k,
                    k -> {
                      ...
                      return new PlaybookRoleModel(
                          ...
                          roleSource.enable(),
                          roleSource.roleOwner() != null
                              ? roleSource.roleOwner()
                              : playbookModel.roleOwner(),
                          roleSource.unsupportedRevokeBehavior() != null
                              ? roleSource.unsupportedRevokeBehavior()
                              : playbookModel.unsupportedRevokeBehavior());
                    }));
    return new PlaybookModel(
        roles, playbookModel.roleOwner(), playbookModel.unsupportedRevokeBehavior());
  }
  ...
  public static List<PlaybookPrivilegeGrant> consolidatePrivileges(
    ...
    return groupedRoles.values().stream()
        .map(
            rl -> {
        ...
        .sorted(Comparator.comparing(Record::toString))
        .toList();
  }
}
