...
public class PrivateKeyReader {
    ...
    if (pemObject instanceof PKCS8EncryptedPrivateKeyInfo encryptedPrivateKeyInfo) {
      ...
      if (passphrase == null) {
        throw new PrivateKeyParseException(
            "A passphrase is required when "
                + "decoding an encrypted private key. Either decrypt the key "
                + "ahead of time or provide a private key password.");
      }
      ...
      throw new PrivateKeyParseException(
          String.format("Unsupported class %s for pemObject", pemObject.getClass().getName()));
    }
  ...
  public static PrivateKey privateKey(@NotEmpty byte[] value, String passphrase)
      throws PrivateKeyParseException {
    ...
    try (PEMParser pemParser = new PEMParser(new StringReader(privateKeyString))) {
      ...
      throw new PrivateKeyParseException(
          "An error occurred while reading the private key " + "string", e);
    }
  }
