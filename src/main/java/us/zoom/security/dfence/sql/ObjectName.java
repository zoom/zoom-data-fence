package us.zoom.security.dfence.sql;
...
import us.zoom.security.dfence.sql.error.RbacAntlrErrorListener;
import us.zoom.security.dfence.sql.models.SqlObject;
import us.zoom.security.dfence.sql.parser.*;
import us.zoom.security.dfence.sql.visitors.objectname.SqlObjectObjectNameVisitor;
import us.zoom.security.dfence.sql.visitors.showgrantsprocedurename.SqlObjectShowGrantsProcedureNameVisitor;
...
public class ObjectName {
    ...
    public static SqlObject parseSqlObject(String objectName) {
        ...
        try {
            ...
            throw new ObjectNameException(
                    String.format("Object %s is not a valid SQL object name. %s", objectName, e),
                    e);
        }
    }

    public static SqlObject parseSqlObjectFromShowGrantsProcedureName(String showGrantsProcedureName) {
        SqlObject outerSqlObject = parseSqlObject(showGrantsProcedureName);
        if (outerSqlObject.getNames().size() != 3) {
            throw new ObjectNameException(String.format(
                    "Procedure name %s has %s parts. 3 parts expected.",
                    showGrantsProcedureName,
                    outerSqlObject.getNames().size()));
        }
        String callableName = outerSqlObject.getNames().get(2);
        RbacAntlrErrorListener errorListener = new RbacAntlrErrorListener();
        ShowGrantsProcedureNameLexer lexer = new ShowGrantsProcedureNameLexer(CharStreams.fromString(callableName));
        lexer.addErrorListener(errorListener);
        CommonTokenStream tokens = new CommonTokenStream(lexer);
        ShowGrantsProcedureNameParser parser = new ShowGrantsProcedureNameParser(tokens);
        parser.addErrorListener(errorListener);
        ShowGrantsProcedureNameVisitor<SqlObject> visitor = new SqlObjectShowGrantsProcedureNameVisitor();
        try {
            ...
            throw new ObjectNameException(
                    String.format("Object %s is not a valid SQL show grants procedure callable name.",
                            callableName),
                    e);
        }
    }
    ...
    public static String procedureArgumentsToTypes(String arguments) {
        ...
        List<String> argumentBlocks = StringPattern.splitIgnore(
        ...
        for (int i = 0; i < argumentBlocks.size(); i++) {
            ...
            if (argumentBlockMatcher.find()) {
                ...
                throw new ObjectNameException(String.format(
                        "Unable to find argument data type within argument %s",
                        argumentBlocks.get(i)));
            }
        }
