...
public class SqlDataTypeObjectNameVisitor extends ObjectNameBaseVisitor<SqlDataType> {
    ...
    public SqlDataType visitArgument(ObjectNameParser.ArgumentContext ctx) {
        ...
        if (ctx.argumentParams() != null && ctx.argumentParams().INTEGER() != null && !ctx.argumentParams().INTEGER()
                .isEmpty()) {
            sqlDataType.getParams().addAll(ctx.argumentParams().INTEGER().stream().map(ParseTree::getText).toList());
        }
        if (ctx.args() != null && ctx.args().argument() != null && !ctx.args().argument().isEmpty()) {
            sqlDataType.getColumnTypes().addAll(ctx.args().argument().stream().map(x -> x.accept(this)).toList());
        }
        return sqlDataType;
    }
}
