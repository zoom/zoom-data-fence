...
import us.zoom.security.dfence.profile.Profile;
...
public class PlaybookServiceBuilder {
    ...
    public PlaybookServiceBuilder setPlaybookYamlStrings(File playbookFile) {
        ...
        if (playbookFile.isFile()) {
            ...
            try {
                ...
                throw new RbacDataError(String.format("Unable to parse playbook file %s. %s", playbookFile, e));
            }
        } else {
            ...
            try {
                List<String> yamlStrings = Files.find(
                        playbookFile.toPath(),
                        20,
                        (filePath, fileAttr) -> pathMatcher.matches(filePath) && fileAttr.isRegularFile()).map(x -> {
                    try {
    ...
    public PlaybookServiceBuilder setProfilesYamlString(File profilesFile) {
        try {
            ...
            throw new RbacDataError(String.format("Unable to parse profiles file %s. %s", profilesFile, e));
        }
    }
    ...
    public PlaybookServiceBuilder putAllVariablesFile(File variablesFile) throws RbacDataError {
        ...
        if (variablesFile != null) {
            try {
                ...
                throw new RbacDataError(String.format("Unable to read variables file %s. %s", variablesFile, e));
            }
            try {
                this.putAllVariables(VariableParser.parseVariables(variablesYamlString));
            } catch (JsonProcessingException e) {
                throw new RbacDataError(String.format(
                        "Unable to parse yaml file %s as valid yaml. %s",
                        variablesFile,
                        e));
            }
        }
    ...
    public PlaybookService build() {
        ...
        List<PlaybookModel> playbookModels = playbookYamlStrings.stream().map(x -> Playbook.parse(x, variables))
                .toList();
